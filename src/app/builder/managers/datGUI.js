...
export default class DatGUI {
  ...
  load(main) {
    ...
    meshFolder.add(Config.mesh, 'translucent', true).name('Translucent').onChange((value) => {
      if(value) {
        app.field.bays.forEach((bay) => {
          bay.obj.material.transparent = true;
          bay.obj.material.opacity = 0.5;
        })
      } else {
        app.field.bays.forEach((bay) => {
          bay.obj.material.opacity = 1;
        })
      }
    });
    meshFolder.add(Config.mesh, 'wireframe', true).name('Wireframe').onChange((value) => {
      app.field.bays.forEach((bay) => {
        bay.obj.material.wireframe = value;
      })
    });
    meshFolder.add(Config.mesh, 'scaleX', 1, 5).name('Scale X').onChange((value) => {
      mesh.scale.x = value;
      mesh.line.scale.x = value;
    });
    meshFolder.add(Config.mesh, 'positionX', 0, 10).name('Position X').onChange((value) => {
      mesh.position.x = value;
      mesh.line.position.x = value;
    });
