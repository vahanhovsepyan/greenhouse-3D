...
export default class Field {
  ...
  updatePlanePosition() {
    if (this.bays[0]) {
      let baysWidth =
        this.direction == "vertical"
          ? this.bays[0].obj.size.z
          : this.bays[0].obj.size.x;
      this.mesh.position.z =
        this.bays[0].obj.position.z - (this.bays.length - 1) * baysWidth;
  ...
  add(bay) {
    ...
    bay.obj.position.set(
      calculatePositionX,
      bay.obj.position.y || 0,
      calculatePositionZ
    );
    bay.index = baysCount;
    bay.field = this;
