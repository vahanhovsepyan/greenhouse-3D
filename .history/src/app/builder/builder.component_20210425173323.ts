...
export class BuilderComponent implements OnInit {
  ...
  setDimensions() {
    ...
    if(this.model.dimensions.heat.volume < 250 || this.model.dimensions.heat.volume > 10000) {
      this.toast.open(`HST volume should be between 250 and 10000m3.`);
      return;
    }
  ...
  render() {
    ...
    this.fields.forEach((field) => {
      if(field.ghost) ghostField = field;
  ...
  buildData() {
    ...
    this.fields.forEach((field) => {
      let block = {
        type: field.type,
        x: field.group.position.x,
        z: field.group.position.z,
        direction: field.direction,
        elements: []
      };
      
      // Adding objects into elements 
      field.bays.forEach((bay) => {
        let element = {
          x: bay.obj.position.x,
          z: bay.obj.position.z,
          size: bay.obj.size,
          marked: bay.marked,
          stepSize: bay.stepSize
        };
